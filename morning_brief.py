...
# ── 할일 후보 추출 (어제 기록에서) ───────────────────────────────
def get_todo_candidates(daily_text: str) -> str:
    """어제 WorkFlowy 기록에서 Jay가 직접 해야 할 액션을 추출 (표시 전용)."""
    try:
...
할일 후보:"""
        ...
        if not lines:
            return "  (발견된 할일 없음)"
        return "\n".join(f"  · {escape(l)}" for l in lines[:4])
    except Exception as e:
...
def get_daniel_section() -> str:
    try:
        ...
        # 할일 후보 (어제 기록에서 추출 — 표시 전용, 다음 단계에서 버튼 연결 예정)
        todo_candidates = get_todo_candidates(daily_text)
